...
class Iamporter {
    ...
    return new Promise((resolve, reject) => {
      this.client.request(spec)
        .then(res => {
          ...
          if (data.code !== 0) {
            return reject(new IamporterError(data.message, data));
          } else {
            return resolve({
              code: data.code,
        ...
        .catch(err => {
          ...
          case HTTP_UNAUTHORIZED:
            return reject(new IamporterError('아임포트 API 인증에 실패하였습니다.', data));
          case HTTP_NOT_FOUND:
            return resolve({
              code: 0,
              message: data.message,
  ...
  isExpiredToken() {
    return !this.expireAt || this.expireAt <= _.floor(_.now() / 1000);
  }
  ...
  createSubscription(data) {
    ...
    const spec = {
      method: 'POST',
      url: `/subscribe/customers/${data.customerUid}`,
      data: data
    };
